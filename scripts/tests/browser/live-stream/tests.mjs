...
export async function exercise({client, sessionId, ready, directory, artifacts, pauseManager, resumeManager}) {
  ...
  try {
    ...
    await prompt("provider error");
    await wait('document.querySelector("#live-status")?.textContent === "Ready" && document.querySelector("#live-error")?.textContent.includes("Reasona failed: upstream returned 503")', "public provider error is visible");
    const failed = await snapshot();
    const failedNotice = await evaluate('document.querySelector("#live-error").textContent');
    assert(failed.error.includes("Reasona failed: upstream returned 503") && failed.error.includes("Review the saved session") && !failed.error.includes("agent: provider chat:") && failedNotice.includes("Reasona failed: upstream returned 503") && failedNotice.includes("Review the saved session") && !failedNotice.includes("agent: provider chat:"), "failed prompt notice must show bounded TUI-equivalent detail plus explicit retry guidance");
    assert(await evaluate('!!document.querySelector("[data-live-error-dismiss]")'), "failed prompt notice must offer an explicit dismiss control");
    const dismissPosts = requests.filter(request => request.method === "POST").length;
    await click("[data-live-error-dismiss]");
    assert(await evaluate('document.querySelector("#live-error").hidden && document.querySelector("#live-status")?.textContent === "Ready" && !document.querySelector("#live-send").disabled && document.activeElement === document.querySelector("#live-prompt")') && requests.filter(request => request.method === "POST").length === dismissPosts, "dismissing the error must be local presentation only, return focus to the composer, and leave the runtime ready");
    await evaluate(`window.SnowLiveView.updateChrome({error:${JSON.stringify(failed.error)}})`);
    await delay(100);
    assert(await evaluate('document.querySelector("#live-error").hidden'), "a repeated snapshot of the same error must not immediately reopen a dismissed notice");
    await prompt("provider error");
    await wait('!document.querySelector("#live-error")?.hidden && document.querySelector("#live-error")?.textContent.includes("Reasona failed: upstream returned 503")', "a new failed prompt shows the same diagnostic again");
    assert(await evaluate('!!document.querySelector("[data-live-error-dismiss]")'), "a repeated failure retains its dismiss control");
    await shot("08-provider-error");
    await click("[data-runtime-close]");
