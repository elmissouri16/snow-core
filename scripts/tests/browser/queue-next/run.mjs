...
const temporary = await mkdtemp(join(tmpdir(), "snow-queue-next-"));
const evidenceDirectory = process.env.SNOW_QUEUE_NEXT_EVIDENCE === "1" ? await mkdtemp(join(tmpdir(), "snow-queue-next-evidence-")) : null;
...
try {
  ...
  const fixture = manifest.find(item => item.name === "workflow-queue");
  if (!fixture) throw new Error("Missing production workflow fixture");
  const html = await readFile(join(directory, "workflow-queue.html"));
  if (!html.includes('data-streaming="true"')) throw new Error("Production queue fixture must enable real SSE");
  for (const capability of ['data-turn-cancel="true"', 'data-message-edit-enabled="true"', 'data-message-regenerate-enabled="true"']) if (!html.includes(capability)) throw new Error(`Production queue fixture missing ${capability}`);
  if (!html.includes('data-queue-next-enabled="true"')) throw new Error("Go workflow fixture must set QueueNextEnabled; tests never synthesize production controls");
  ...
  const unsupported = await readFile(join(directory, "workflow-cancel.html"));
  const files = new Map([["/workflow-queue.html", html], ["/workflow-cancel.html", unsupported], ["/saved-user.html", savedUser]]);
  let bytes = 0;
  ...
  const deadline = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("Queue next checks exceeded 300s")), 300000); });
  await Promise.race([deadline, (async () => {
    ...
    for (const width of (process.env.SNOW_QUEUE_NEXT_WIDTH ? [Number(process.env.SNOW_QUEUE_NEXT_WIDTH)] : [320, 1280])) for (const height of [740]) for (const theme of (process.env.SNOW_QUEUE_NEXT_THEME ? [process.env.SNOW_QUEUE_NEXT_THEME] : ["dark", "light"])) {
      reports++;
      ...
      const {identifier} = await send("Page.addScriptToEvaluateOnNewDocument", {source: `
        ...
        window.queueNextErrors=[]; window.queueNextInvalidEvents=0;
        addEventListener("error", e => queueNextErrors.push(e.message));
        addEventListener("unhandledrejection", e => queueNextErrors.push(String(e.reason)));
        document.addEventListener("invalid", () => queueNextInvalidEvents++, true);
      `});
      const navigate = async (page = "workflow-queue") => {
        mock.state.reset();
        ...
        await wait('document.readyState === "complete" && document.querySelector("#live-connection")?.textContent === "Live"', "production fixture connected");
      };
      const captures = [];
      ...
      } catch (error) { const passed = (error.results?.length || 0) - (error.failures?.length || 0); total += error.results?.length || 0; assertionFailures += error.failures?.length || 0; failures += 1 + (error.failures?.length || 0); mock.state.release(); console.log(JSON.stringify({width, height, theme, completedAssertions: error.results?.length || 0, passedAssertions: passed, assertionFailures: error.failures || [], fatal: error.message, stack: error.stack, requests: mock.state.requests.map(({path}) => path)})); }
      await send("Page.removeScriptToEvaluateOnNewDocument", {identifier});
    }
    if (external.length) { failures++; console.error("Unexpected external requests", external); }
    console.log(`${total - assertionFailures} assertions passed; ${failures} failures across ${reports} queue-next reports.`);
