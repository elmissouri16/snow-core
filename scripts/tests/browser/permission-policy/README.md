...
instance/session identity, CSRF, policy, and Allow-only acknowledgement). The additional mode checks use real
HTMX 2.0.10 XHR requests and the production SSE parser against a local stream.
`fetch`, HTMX, DOM markup, and CSS are not replaced. Telemetry checks wrap the
...
checks. Native HTMX reads exercise preserved search/scroll/focus, superseded
navigation, delayed responses, Rename focus return, unchanged row updates, and
live-state survival after a rejected swap. The mock reselects `#workspace` from
exported full documents to match production fragment navigation; newly inserted
links are exercised only after HTMX settlement. No navigation markup is fabricated.
...
- Repeated held Default/Plan POSTs use real HTMX with exact allowlisted fields,
...
- Malformed or interrupted acknowledgements still fail closed without application
  replay. Terminal SSE replacement revokes authority during an in-flight HTMX
  request; a late old-lifetime reply cannot re-enable the replaced conversation.

- Native HTMX model discovery holds the response through initial load, identical
...
- Compact telemetry rows, Details/Back/Escape, unknown versus verified-zero cost,
  ...
  and expanded notes at the same viewport. Instrumentation verifies zero menu
  reconciliations/DOM mutations for unchanged metrics and unrelated settings,
  text-only updates in retained nodes for changed values, and no extra requests.
