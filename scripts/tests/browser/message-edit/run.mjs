...
const temporary = await mkdtemp(join(tmpdir(), "snow-message-edit-"));
const evidenceDirectory = process.env.SNOW_MESSAGE_EDIT_EVIDENCE === "1" ? await mkdtemp(join(tmpdir(), "snow-message-edit-evidence-")) : null;
...
try {
  ...
  if (!html.includes('data-streaming="true"')) throw new Error("Production edit fixture must enable real SSE");
  if (!html.includes('data-message-edit-enabled="true"')) throw new Error("Go workflow fixture must set MessageEditEnabled; tests never synthesize production controls");
  ...
  const files = new Map([["/workflow-edit.html", html], ["/workflow-cancel.html", unsupported], ["/saved-user.html", savedUser]]);
  let bytes = 0;
  ...
  const deadline = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("Message edit checks exceeded 240s")), 240000); });
  await Promise.race([deadline, (async () => {
    ...
    for (const width of (process.env.SNOW_MESSAGE_EDIT_WIDTH ? [Number(process.env.SNOW_MESSAGE_EDIT_WIDTH)] : [320, 1280])) for (const height of [740]) for (const theme of (process.env.SNOW_MESSAGE_EDIT_THEME ? [process.env.SNOW_MESSAGE_EDIT_THEME] : ["dark", "light"])) {
      reports++;
      ...
      const {identifier} = await send("Page.addScriptToEvaluateOnNewDocument", {source: `
        ...
        window.messageEditErrors=[]; window.messageEditInvalidEvents=0;
        addEventListener("error", e => messageEditErrors.push(e.message));
        addEventListener("unhandledrejection", e => messageEditErrors.push(String(e.reason)));
        document.addEventListener("invalid", () => messageEditInvalidEvents++, true);
      `});
      const navigate = async (page = "workflow-edit") => {
        mock.state.reset();
      ...
      } catch (error) { failures++; mock.state.release(); console.log(JSON.stringify({width, height, theme, fatal: error.message, stack: error.stack, requests: mock.state.requests.map(({path}) => path)})); }
      await send("Page.removeScriptToEvaluateOnNewDocument", {identifier});
    }
    if (external.length) { failures++; console.error("Unexpected external requests", external); }
    console.log(`${total - assertionFailures} assertions passed; ${failures} failures across ${reports} message-edit reports.`);
