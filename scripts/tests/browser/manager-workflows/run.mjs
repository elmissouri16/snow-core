...
async function run(width, theme, binary) {
  const directory = await mkdtemp(join(tmpdir(), "snow-manager-workflows-"));
  ...
  try {
    ...
    return await Promise.race([deadline, (async () => {
      ...
      delete ready.cookie;
      const result = await exercise({client, sessionId, ready, directory, width, theme});
...
const build = await mkdtemp(join(tmpdir(), "snow-manager-workflows-build-"));
let passed = 0, failures = 0, reports = 0;
try {
  const binary = join(build, "snow-fixture.test"); await compile(binary);
  const widths = process.env.SNOW_MANAGER_WORKFLOWS_WIDTH ? [Number(process.env.SNOW_MANAGER_WORKFLOWS_WIDTH)] : [320, 1280];
  const themes = process.env.SNOW_MANAGER_WORKFLOWS_THEME ? [process.env.SNOW_MANAGER_WORKFLOWS_THEME] : ["dark", "light"];
...
} catch (error) { failures++; console.error(`manager-workflows: ${error.message}`); }
