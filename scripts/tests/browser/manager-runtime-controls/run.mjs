...
const artifacts = process.env.SNOW_RUNTIME_CONTROLS_ARTIFACTS ? resolve(process.env.SNOW_RUNTIME_CONTROLS_ARTIFACTS) : null;
const timeoutMS = 155000;
...
async function run(width, theme, binary) {
  const directory = await mkdtemp(join(tmpdir(), "snow-manager-runtime-controls-"));
  ...
  try {
    ...
    return await Promise.race([deadline, (async () => {
      ...
      manager = spawn(binary, ["-test.run=^TestWebRuntimeControlsBrowserFixture$", "-test.count=1", "-test.timeout=180s"], {
        cwd: repository, env: {PATH: process.env.PATH || "/usr/bin:/bin", TMPDIR: tmpdir(), SNOW_WEB_RUNTIME_CONTROLS_BROWSER_DIR: directory}, stdio: ["pipe", "pipe", "pipe"]
      });
      ...
      const ready = await new Promise((resolve, reject) => {
        ...
        manager.stdout.on("data", chunk => {
          ...
          while ((index = buffer.indexOf("\n")) >= 0) {
            ...
            if (line.startsWith("SNOW_RUNTIME_CONTROLS_READY ")) {
              try { resolve(JSON.parse(line.slice("SNOW_RUNTIME_CONTROLS_READY ".length))); }
      ...
      const secrets = [ready.cookie]; delete ready.cookie;
      const result = await exercise({client, sessionId, ready, directory, width, theme, artifacts, secrets});
...
const build = await mkdtemp(join(tmpdir(), "snow-manager-runtime-controls-build-"));
let passed = 0, failures = 0, reports = 0;
try {
  if (artifacts) await mkdir(artifacts, {recursive: true, mode: 0o700});
  const binary = process.env.SNOW_RUNTIME_CONTROLS_BINARY ? resolve(process.env.SNOW_RUNTIME_CONTROLS_BINARY) : join(build, "snow-fixture.test"); if (!process.env.SNOW_RUNTIME_CONTROLS_BINARY) await compile(binary);
  const widths = process.env.SNOW_RUNTIME_CONTROLS_WIDTH ? [Number(process.env.SNOW_RUNTIME_CONTROLS_WIDTH)] : [320, 1280];
  const themes = process.env.SNOW_RUNTIME_CONTROLS_THEME ? [process.env.SNOW_RUNTIME_CONTROLS_THEME] : ["dark", "light"];
...
} catch (error) { failures++; console.error(`manager-runtime-controls: ${error.message}`); }
