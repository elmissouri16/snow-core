...
const temporary = await mkdtemp(join(tmpdir(), "snow-message-regenerate-"));
const evidenceDirectory = process.env.SNOW_MESSAGE_REGENERATE_EVIDENCE === "1" ? await mkdtemp(join(tmpdir(), "snow-message-regenerate-evidence-")) : null;
...
try {
  ...
  const fixture = manifest.find(item => item.name === "workflow-regenerate");
  if (!fixture) throw new Error("Missing production workflow fixture");
  const html = await readFile(join(directory, "workflow-regenerate.html"));
  if (!html.includes('data-streaming="true"')) throw new Error("Production regenerate fixture must enable real SSE");
  for (const capability of ['data-turn-cancel="true"', 'data-message-edit-enabled="true"']) if (!html.includes(capability)) throw new Error(`Production regenerate fixture missing ${capability}`);
  if (!html.includes('data-message-regenerate-enabled="true"')) throw new Error("Go workflow fixture must set MessageRegenerateEnabled; tests never synthesize production controls");
  ...
  const files = new Map([["/workflow-regenerate.html", html], ["/workflow-cancel.html", unsupported], ["/saved-user.html", savedUser]]);
  let bytes = 0;
  ...
  const deadline = new Promise((_, reject) => { timer = setTimeout(() => reject(new Error("Message regenerate checks exceeded 240s")), 240000); });
  await Promise.race([deadline, (async () => {
    ...
    for (const width of (process.env.SNOW_MESSAGE_REGENERATE_WIDTH ? [Number(process.env.SNOW_MESSAGE_REGENERATE_WIDTH)] : [320, 1280])) for (const height of [740]) for (const theme of (process.env.SNOW_MESSAGE_REGENERATE_THEME ? [process.env.SNOW_MESSAGE_REGENERATE_THEME] : ["dark", "light"])) {
      reports++;
      ...
      const {identifier} = await send("Page.addScriptToEvaluateOnNewDocument", {source: `
        ...
        window.messageRegenerateErrors=[]; window.messageRegenerateInvalidEvents=0;
        addEventListener("error", e => messageRegenerateErrors.push(e.message));
        addEventListener("unhandledrejection", e => messageRegenerateErrors.push(String(e.reason)));
        document.addEventListener("invalid", () => messageRegenerateInvalidEvents++, true);
      `});
      const navigate = async (page = "workflow-regenerate") => {
        mock.state.reset();
    ...
    console.log(`${total - assertionFailures} assertions passed; ${failures} failures across ${reports} message-regenerate reports.`);
