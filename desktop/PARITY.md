...
| Project session inventory | Persistent current-project sidebar and `/sessions` panel share the bounded `sessions_list` projection; silent readiness/mutation refreshes do not force the panel open or erase an existing error | `silent_session_inventory_refresh_updates_sidebar_without_opening_management_panel`, `silent_session_inventory_completion_preserves_existing_error`; Go inventory tests | Complete |
...
| Restored historical images/tool cards | Protocol validates/decodes bounded public image and tool display blocks; the workspace pairs tool results with matching calls and renders compact collapsed status cards whose fenced input/output details use a fixed-height internal scroller | `typed_history_preserves_safe_images_and_tool_cards`, `restored_tool_results_join_their_tool_call_card`, `collapsed_tool_card_summaries_are_single_line_and_bounded`, rejection/bounds tests | Complete |
