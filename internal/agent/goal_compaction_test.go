...
func TestMailboxGoalToolCyclePermissionRequiresAutomaticGoalOrigin(t *testing.T) {
	a, c, _ := goalAgent(t, &scriptedProvider{})
	goal, err := c.Create("continue", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	tests := []struct {
		...
		{name: "automatic goal", origin: "goal", goal: goal, want: true},
		{name: "automatic goal compaction", origin: "compact", goal: goal, want: true},
		{name: "user turn with goal", origin: "user", goal: goal, want: false},
		{name: "subagent", origin: "subagent", goal: goal, want: false},
		{name: "manual compaction", origin: "compact", want: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			a.mu.Lock()
			a.turnOrigin = tt.origin
			a.goalAtTurn = tt.goal
			a.mu.Unlock()
			if got := a.mailboxGoalToolCyclesAllowed(); got != tt.want {
				t.Fatalf("mailbox goal-cycle permission=%v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestGoalAutoCompactsMailboxHeadedActiveToolChain(t *testing.T) {
	p := &scriptedProvider{}
	a, c, st := goalAgent(t, p)
	for i := range 3 {
		message := protocol.Message{
			...
			Content: []protocol.ContentBlock{protocol.NewTextBlock("mailbox update")},
		}
		if err := st.Append(session.Entry{Type: session.EntryMessage, ID: message.ID, Message: &message}); err != nil {
			t.Fatal(err)
		}
	...
	a.model.ContextWindow = 100
	a.opts.Compaction = CompactionOptions{RetainTokens: 1, MinRetainedTurns: 2, SummaryMaxTokens: 128, Fallback: "local", AutoThresholdPercent: 90}
	g, err := c.Create("compact mailbox-headed cycles and continue", nil, false)
	if err != nil {
		t.Fatal(err)
	}
	p.scripts = [][]protocol.StreamEvent{
		{{Type: protocol.EvStreamToolCallDone, ToolCallID: "check-1", ToolName: "get_goal", Arguments: []byte(`{}`)}, {Type: protocol.EvStreamDone, StopReason: protocol.StopToolUse}},
		{{Type: protocol.EvStreamToolCallDone, ToolCallID: "check-2", ToolName: "get_goal", Arguments: []byte(`{}`)}, {Type: protocol.EvStreamDone, StopReason: protocol.StopToolUse}},
		{{Type: protocol.EvStreamToolCallDone, ToolCallID: "check-3", ToolName: "get_goal", Arguments: []byte(`{}`)}, {Type: protocol.EvStreamUsage, Usage: &protocol.Usage{Input: 90, Output: 1, Total: 91}}, {Type: protocol.EvStreamDone, StopReason: protocol.StopToolUse}},
		{{Type: protocol.EvStreamTextDelta, Text: "goal summary"}, {Type: protocol.EvStreamDone, StopReason: protocol.StopStop}},
		{{Type: protocol.EvStreamToolCallDone, ToolCallID: "done", ToolName: "update_goal", Arguments: []byte(`{"goal_id":"` + g.GoalID + `","status":"complete"}`)}, {Type: protocol.EvStreamDone, StopReason: protocol.StopToolUse}},
		{{Type: protocol.EvStreamDone, StopReason: protocol.StopStop}},
	}
	...
	for a.IsRunning() || func() bool { a.mu.RLock(); defer a.mu.RUnlock(); return a.autoRunning }() {
		if time.Now().After(deadline) {
			t.Fatal("mailbox-headed goal compaction did not finish")
		}
		time.Sleep(time.Millisecond)
	}
	goal, err := c.Get()
	...
	if goal.Status != protocol.GoalComplete || p.call != 6 {
		t.Fatalf("goal=%+v calls=%d requests=%+v", goal, p.call, p.requests)
	}
	if !strings.Contains(p.requests[3].System, "working-state checkpoint") {
		t.Fatalf("fourth request was not the compaction summary: %q", p.requests[3].System)
	}
	continued := p.requests[4].Messages
	if len(continued) == 0 || continued[0].Role != protocol.RoleCustom {
		t.Fatalf("continued goal lacks compacted checkpoint: %+v", continued)
	}
}
