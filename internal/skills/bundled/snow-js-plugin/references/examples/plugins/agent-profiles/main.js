...
(() => {
  ...
  function register(snow2) {
    ...
    snow2.registerView({ name: "profile", title: "Agent profile", placement: "footer" });
    async function show(ctx, name) {
      if (ctx.ui.available) {
    ...
    snow2.registerCommand({
      name: "profile",
      alias: "profile",
      description: "Select a branch-local agent profile",
      argumentHint: "reviewer | architect | debugger | off",
    ...
    snow2.registerHook("before_request", (request) => {
