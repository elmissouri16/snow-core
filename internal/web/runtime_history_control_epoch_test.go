...
func TestHistoryBranchForkEpochOrderingKeepsNextPromptAndGoalAttention(t *testing.T) {
	for _, ordering := range []string{"epoch-before-ack", "epoch-after-ack"} {
		for _, next := range []string{"prompt", "goal"} {
			t.Run(ordering+"/"+next, func(t *testing.T) {
				m, p, before, log := openHistoryControlRuntime(t, ordering)
				...
				go func() {
					s, err := m.HistoryBranchFork(t.Context(), p.ID, before.InstanceID, request)
					done <- result{s, err}
				}()
				...
				if next == "prompt" {
					...
					_, err = m.RunGoal(t.Context(), p.ID, current.InstanceID, RuntimeGoalRunInput{ExpectedRevision: current.Revision, RPCGoalRunParams: protocol.RPCGoalRunParams{Action: "create", SessionID: current.SessionID, BranchID: "forked", ExpectedTipID: "target-tip", Objective: "explicit goal after restore"}})
				}
				if err != nil {
					t.Fatal(err)
				}
