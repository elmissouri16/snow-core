...
func DefaultSubagents() SubagentConfig {
	return SubagentConfig{MaxConcurrentThreads: 4, MaxAgentsPerSession: 32, MaxDepth: 1,
		...
		DefaultRole: "general", Roles: map[string]AgentRole{
			"general":     {Description: "General shell-capable investigation (bash remains permission-gated; file mutation is disabled by default)", Tools: []string{"read", "grep", "glob", "artifact_read", "artifact_grep", "activate_skill", "deactivate_skill", "read_skill_resource", "snow_plugin_docs", "bash"}},
			"explorer":    {Description: "Narrow read-only codebase investigation", Tools: []string{"read", "grep", "glob", "artifact_read", "artifact_grep", "activate_skill", "deactivate_skill", "read_skill_resource", "snow_plugin_docs"}},
...
func (c SubagentConfig) ValidateSubagents() error {
	...
	childTools := map[string]bool{"read": true, "grep": true, "glob": true, "artifact_read": true, "artifact_grep": true, "activate_skill": true, "deactivate_skill": true, "read_skill_resource": true, "snow_plugin_docs": true, "write": true, "edit": true, "bash": true}
