...
interface SnowToolRestriction {allow?:string[]; deny?:string[];}
interface SnowWorkflowUpdate {set?:Record<string,JSONValue>; delete?:string[]; toolRestriction?:SnowToolRestriction|null;}
interface SnowToolInfo {name:string; source:string; effect:string; allowed:boolean; reason?:string;}
interface SnowWorkflowReceipt {branchId:string; tipId:string;}
interface SnowSessionChange {operation:string;oldSessionId:string;newSessionId:string;oldBranchId:string;newBranchId?:string;fromEntryId?:string;}
interface SnowCompactionPlan {trigger:string;boundaryId:string;summarizedMessages:number;retainedMessages:number;}
interface SnowContext {
 ...
 tools:{call(name:string,args?:Params):Promise<SnowResult>; list():Promise<SnowToolInfo[]>; restrict(args:SnowToolRestriction):Promise<SnowWorkflowReceipt>; clearRestriction():Promise<SnowWorkflowReceipt>};
 /** Writes commit immediately; only idle root commands may write workflow state. */
 workflow:{get(args:{key:string}):Promise<JSONValue>;set(args:{key:string;value:JSONValue}):Promise<SnowWorkflowReceipt>;delete(args:{key:string}):Promise<SnowWorkflowReceipt>;update(args:SnowWorkflowUpdate):Promise<SnowWorkflowReceipt>};
...
interface SnowHook {phase:"before_prompt"|"before_request"|"before_tool"|"after_tool"|"before_session_change"|"before_compaction";workflow?:Record<string,JSONValue>;sessionChange?:SnowSessionChange;compaction?:SnowCompactionPlan;text?:string;tool?:string;arguments?:Params;content?:{type:"text";text:string}[];isError?:boolean;context?:{source?:string;text:string}[];agent?:Params;}
interface SnowHookResult {text?:string;arguments?:Params;content?:{type:"text";text:string}[];context?:{text:string}[];block?:string;}
/** Factories run synchronously during registration, before host I/O is bound. */
interface SnowAPI {
 ...
 registerHook(phase:SnowHook["phase"],handler:(request:SnowHook,ctx:SnowContext)=>SnowHookResult|Promise<SnowHookResult>,options?:{includeSubagents?:boolean;workflowKeys?:string[]}):void;
