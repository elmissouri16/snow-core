...
| Known reproducible defects | BUG-086, BUG-175 and BUG-218 are resolved. Fix the reopened [BUG-180](../bugs.md#bug-180-sidebar-navigation-and-row-controls-lose-focus-and-list-state): the current React navigation owner loses destination-row focus, open search/filter state and the user's post-navigation focus choice. Keep it open until all eight permission-policy reports pass again. |
...
native runtime-control matrix, all 84 runtime-layout reports and the Go web
package.
