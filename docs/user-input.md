...
Snow exposes `ask_user` as a direct, always-loaded built-in tool. It is not
indexed by deferred discovery. An explicit CLI/SDK tool allowlist can still
exclude it.

## Request shape

A call contains one to three questions. IDs are unique snake_case strings.
Headers are short labels. A question without `options` is free-form; a question
with options must contain two or three mutually exclusive choices.
  ...
  "questions": [
    {
      ...
      "options": [
        ...
        {"label": "Text", "description": "Human-readable prose."}
      ]
    },
    {
      "id": "notes",
...
```json
{"answers":[{"id":"format","answer":"JSON"},{"id":"notes","answer":"Keep comments"}]}
```
...
These one-shot surfaces have no input channel. If the model invokes `ask_user`,
the call immediately becomes an unavailable-input tool error and the agent may
continue or finish normally. The process never blocks waiting for input.
...
request. `ask_user` is interaction/read risk and does not trigger the mutation
