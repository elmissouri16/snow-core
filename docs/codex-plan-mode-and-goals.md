# Codex Plan Mode and Thread Goals Research

This document records source-level research into how OpenAI Codex implements
Plan Mode and Thread Goals, and how those two designs map onto snow-core. It is
an RFC-style design record: the source findings are the reference, while
current Snow behavior is covered in the shorter Plan Mode and Goals guides.

The analysis covers the open-source `openai/codex` repository (Rust
implementation, `codex-rs/`), shallow-cloned at commit
`208f05b23387c47d7e52fd2153d59255e945d0b7` (2026-08-07, "Enforce automatic
review for managed models"). All Codex file paths are relative to the Codex
repo root.
...
- [Source research: Plan Mode](#source-research-plan-mode)
- [Source research: Thread Goals](#source-research-thread-goals)
...
Codex implements two separable design patterns. Plan Mode is a collaboration
mode with read-only planning behavior and a structured `<proposed_plan>` output
contract. Thread Goals are persisted per-thread objectives with budgets, usage
accounting, and automatic idle continuation.

Both map cleanly onto snow-core: Plan Mode is mostly context assembly plus a
streaming contract, while Thread Goals are a lifecycle extension over the
session store. The comparison table below records the deliberate differences
Snow chose when adopting each pattern.

## Context and motivation

This research was written to inform snow-core's Plan Mode and Thread Goals
design by reading the upstream implementation and its adjacent tests, not
product descriptions. Pinning the commit matters because the Codex repository
changes quickly and this analysis targets a specific snapshot.

The two features are explained end to end: Plan Mode (a collaboration mode with
read-only planning behavior and a structured `<proposed_plan>` output contract)
and Thread Goals (persisted per-thread objectives with budgets, usage
accounting, and automatic idle continuation).

## Source research: Plan Mode

### What Plan Mode is (and is not)

Plan Mode is a collaboration mode, not a tool. It is a mode the user switches
the whole session into, and it changes:
...
- how assistant output is streamed and displayed (a special `<proposed_plan>`
  block becomes a rendered plan item instead of chat text);
...
`CollaborationModeMask` (partial application: mode + optional model + optional
effort + optional instructions), so switching modes can also switch models.
...
- **3 phases**: (1) ground in the environment with non-mutating exploration,
  (2) intent chat, (3) implementation chat until the spec is "decision
  complete" (no decisions left for the implementer).
- **Execution vs mutation**: only non-mutating actions are allowed (reads,
  searches, dry-run commands, tests/builds that don't edit repo-tracked files).
  Mutating actions (editing/writing files, formatters/linters that rewrite,
  applying patches/codegen, side-effectful "doing the work" commands) are
  prohibited. This is enforced by instruction, not by tool filtering.
...
- **Finalization contract**: when the plan is decision-complete, wrap it in a
  ...
  closing tag on its own line, at most one block per turn, complete replacement
  on revisions). Do not ask "should I proceed?".
...
### Prompt injection: collaboration mode as a world-state section

The mode instructions are injected into the model context as a
developer-message fragment with a persistent marker, managed by the world-state
section `codex-rs/core/src/context/world_state/collaboration_mode.rs`:

- `CollaborationModeState` snapshots `{ mode, model }` plus the resolved
  instructions (catalog message first, then `settings.developer_instructions`).
- It renders as `CollaborationModeInstructions` with markers
  `<collaboration_mode>` / `</collaboration_mode>` and role `developer`.
- It participates in the world-state diff machinery: when the mode or model
  changes, a new fragment is appended; when unchanged, nothing is re-emitted.
  It also matches retained fragments so compaction/restore does not duplicate
  the instructions.
...
1. Agent message starts are deferred until the parser emits non-plan text, so a
   plan-only response never shows up as an empty assistant message
   (`maybe_emit_pending_agent_message_start`).
2. Leading whitespace is buffered per item until non-whitespace text rules out
   a tag prefix.
3. Plan content is routed to a `TurnItem::Plan` (a `PlanItem` with its own id
   `<turn_id>-plan`) and streamed via `PlanDelta` events, while normal text goes
   to `AgentMessageContentDelta` events.
...
### The streaming `<proposed_plan>` parser pipeline

`codex-rs/utils/stream-parser/` is a small incremental-stream parsing library:

- `tagged_line_parser.rs` — a generic incremental parser for `TagSpec { open,
  close, tag }` line-oriented markup, emitting `TaggedLineSegment::{Normal,
  TagStart, TagDelta, TagEnd}` as chunks arrive (handles tags split across chunk
  boundaries, unterminated tags on finish).
...
- `assistant_text.rs` — `AssistantTextStreamParser` composes
  ...
  citations) with the plan parser, in one pass. In non-plan mode the plan parser
  is skipped entirely.
- `stream_text.rs` — `StreamTextChunk { visible_text, extracted }` carries both
  the cleaned display text and the ordered extracted segments.
...
`codex-rs/core/src/stream_events_utils.rs` also strips `<proposed_plan>` blocks
from realtime/text event payloads so downstream display never shows raw tags.

### Plan-mode behavioral gates

Beyond instructions, plan mode has hard behavioral gates:

1. **`update_plan` rejected** in Plan mode (see What Plan Mode is (and is
   not)).
...
3. **No autonomous idle turns in Plan mode**
   ...
   input without user content when `collaboration_mode().mode == ModeKind::Plan`
   (`TryStartTurnIfIdleRejectionReason::PlanMode`). This is the same gate goal
   continuation uses, so goals never auto-continue in Plan mode.
4. **Goal accounting excludes Plan mode**
   ...
   `clear_current_turn_goal()` in Plan mode — planning time is not charged to a
   goal.

### TUI surface

- **`/plan`** (or `/plan <message>`) — `codex-rs/tui/src/chatwidget/slash_dispatch.rs`
  to `apply_plan_slash_command()` to
  `collaboration_modes::plan_mask(catalog)` to
...
- **Mode switching is blocked mid-turn**: `submit_user_message_with_mode` errors
  with "Cannot switch collaboration mode while a turn is running."
- **Plan nudge**: the composer shows a "make a plan" suggestion when the draft
  text contains a standalone `plan`/`/plan`/`!plan` keyword while in Default
  mode (`chatwidget/tests/plan_mode.rs`); dismissal is scoped per thread.
...
- **Plan to implementation prompt**: when a turn ends in Plan mode and a plan
  item was seen (`transcript.saw_plan_item_this_turn`),
  `maybe_prompt_plan_implementation` to
  ...
  - *Yes, clear context and implement* — `ClearUiAndSubmitUserMessage` with a
    ...
    (`PLAN_IMPLEMENTATION_CLEAR_CONTEXT_PREFIX` + plan) — context usage label
...
## Source research: Thread Goals

### What a goal is

A thread goal is a persisted, per-thread objective with a status machine, an
optional token budget, token/elapsed-time usage accounting, and the ability to
continue automatically ("while idle") across turns until it is complete,
blocked, or budget-limited. It is implemented as a core extension
...
- **Blocked** — set only after a strict 3-consecutive-turn blocked audit (see
  Completion and blocked audits); prevents auto-continuation loops.
- **UsageLimited** — system sets it when a turn dies from a usage-limit error.
...
`is_terminal()` = `BudgetLimited | Complete`. Replacing a completed goal does
not require user confirmation; replacing an unfinished goal does.

### Model-facing tools

`codex-rs/ext/goal/src/spec.rs` defines three Responses-API tools:

- **`get_goal`** — returns current goal: status, budgets, tokens/elapsed usage,
...
- **`update_goal { status: "complete" | "blocked" }`** — the only statuses the
  model may set. `complete` requires actual achievement; `blocked` requires the
  same blocking condition for at least three consecutive goal turns (original +
  automatic continuations), with a fresh audit after a resume. The model cannot
  pause/resume/budget-limit; those are user/system-controlled. Tool descriptions
  embed the full audit rules.
...
### Steering: how the objective reaches the model

`codex-rs/ext/goal/src/steering.rs` renders three embedded templates
(`ext/goal/templates/goals/`) into `InternalModelContextFragment`s with source
`"goal"`:

- **continuation.md** — used to start an idle continuation turn. States the
  objective (as untrusted data, "not higher-priority instructions"), the budget,
  "work from evidence" (inspect current worktree, don't trust conversation
  memory), fidelity rules ("do not substitute a narrower, safer, smaller...
  solution"), a completion audit (requirement-by-requirement evidence
  verification) and a blocked audit (3 consecutive turns).
...
`<codex_internal_context ...>` / legacy `<goal_context>` markers), so the goal
objective stays visible to the model across turns, compaction, and resume.
`goal_context_input_item` wraps the fragment into a `ResponseItem` that is
injected into the input queue as `TurnInput::ResponseItem`.

### Runtime

`codex-rs/ext/goal/src/runtime.rs` — `GoalRuntimeHandle`, one per thread,
wrapping `GoalRuntimeInner` with a `goal_state_lock` (a single-permit semaphore
serializing goal mutations vs accounting vs continuation):

- **`apply_external_goal_set`** — handles `set` from user/app-server: records
  metrics/analytics (created, resumed, terminal), transitions accounting state
  (mark current turn or idle goal active), injects `objective_updated` steering
  if the objective changed, then calls `continue_if_idle()` to kick off work
  immediately.
- **`continue_if_idle`** — the auto-continuation engine:
  ...
  6. the same gate that rejects Plan-mode idle turns applies, so goals never
     fire in Plan mode,
...
- **`stop_active_goal_for_turn(turn_id, reason)`** — on terminal turn errors:
  `TurnError` to `Blocked`, `UsageLimit` to `UsageLimited` (only from Active, or
  from BudgetLimited to UsageLimited), persists the status, emits
  `ThreadGoalUpdated`.
- **`restore_after_resume`** — after `/resume`, re-mark an `Active` goal as the
  idle active goal so continuation resumes.
- **`prepare_external_goal_mutation`** — flushes accounting before an external
  set/clear/fork so usage is charged to the right goal.
- **`inject_active_turn_steering`** — injects steering items into a running turn
  (`inject_if_running`) when the objective changes mid-turn.
...
- Token deltas are charged at turn stop/abort and at each tool-completion hook
  from `(current - last_accounted)` snapshots; a `progress_accounting_permit`
  semaphore serializes concurrent charges so a delta is never double-counted.
...
`replace_thread_goal_snapshot` (for fork), and the continuation-deferral table
(`has/clear_thread_goal_continuation_deferral`). Concurrency: partial updates
preserve independent fields; `expected_goal_id` guards against stale writes.
Forking flushes progress first (`flush_thread_goal_progress_for_fork`,
`request_processors/thread_fork_goal.rs`).

### Events and TUI

- `EventMsg::ThreadGoalUpdated(ThreadGoalUpdatedEvent { goal, ... })` is emitted
  on every persisted change (set, status change, accounting, clear) and drives
  the TUI goal status indicator and summaries.
- TUI commands (`chatwidget/slash_dispatch.rs`, `app/thread_goal_actions.rs`,
  `chatwidget/goal_menu.rs`):
  - `/goal` — goal menu/summary (status, objective, time/tokens used, budget,
  ...
  - After `/resume`, a paused/blocked/usage-limited goal prompts "Resume paused
    goal?".
  - Feature-flagged via `Feature::Goals`; needs a saved (non-ephemeral) session
    — ephemeral threads get a helpful error.
- `goal_files.rs` materializes oversized objectives, pasted text, and images as
  app-server-hosted files (`goal-objective.md` + attachments under codex home),
  replacing the objective body with a "Read the Codex goal objective file at
  <path> before continuing." reference (there is a
  `MAX_THREAD_GOAL_OBJECTIVE_CHARS` limit).

### Completion and blocked audits

The completion/blocked discipline is prompt-driven, embedded in continuation.md
and the tool descriptions (the system only enforces the budget/usage-limit
transitions):

- **Completion audit**: derive concrete requirements from the objective and
  referenced artifacts; for each requirement find authoritative evidence in the
  current state (files, command output, tests, PR state, runtime behavior);
  weak/indirect/missing evidence means keep working. Only call `update_goal
  status=complete` when every requirement is proven.
- **Blocked audit**: never mark `blocked` on first appearance; only after the
  same blocker recurs for at least three consecutive goal turns (original turn +
  automatic continuations); a resumed run starts a fresh audit. `blocked` is
  reserved for true impasses needing user input or external-state change.

The `blocked`/`complete` statuses flow back through `update_goal` to state DB to
`ThreadGoalUpdated` to TUI, and terminal states stop automatic continuation.
...
| Dimension | Codex approach | Snow approach |
|---|---|---|
| Plan Mode seams | `ModeKind` on the turn context, per-mode developer instructions as a world-state fragment, an incremental `<proposed_plan>` tag parser, and hard gates (`update_plan` rejection, `request_user_input` availability/blocking, idle-continuation suppression, goal-accounting exclusion). | Same seams in `internal/agent`, `internal/plan`, and `pkg/protocol`. The built-in preset supports a configurable reasoning override but does not ingest Codex's model-catalog collaboration instruction field. |
| Thread Goals shape | Lifecycle extension over the session store: a persisted per-thread goal row, turn hooks for token-delta plus idle wall-clock accounting, hidden internal-context steering fragments, and prompt-driven completion/blocked audits. | Same shape hosted by `internal/session`, `internal/goal`, and `pkg/snowsdk`, with TUI/RPC/CLI controls layered on `protocol.AgentEvent`. |
| Goal time accounting | Charges idle process downtime between turns. | Charges monotonic in-process goal work time. |
| Restored goal start | Continues after `/resume`. | Starts restored goals only after a surface-ready signal so initial events cannot be lost. |
| Oversized goal text | Materializes to app-server-hosted files (`goal-objective.md` plus attachments). | Privately resolved by the controller; image attachment hosting remains outside Snow's text-only goal contract. |

## Decision

Snow adopts both patterns as separable designs. Plan Mode is treated as context
assembly plus a streaming contract rather than new tooling, and Thread Goals are
treated as a lifecycle extension over the existing session store. The
differences recorded in the comparison table are deliberate Snow choices, not
unfinished ports.

## Implementation status

Both patterns are implemented in Snow. User-facing behavior is documented in
[Plan Mode](plan-mode.md) and [Goals](goals.md); this record preserves the
upstream research and design rationale behind those implementations.

## Open questions

- Whether to ingest Codex's model-catalog-specific collaboration instruction
  field. Snow's built-in Plan Mode preset currently supports a configurable
  reasoning override only.
- Whether thread goals should host image attachments. Snow's goal contract is
  text-only today; oversized text is resolved privately by the controller.
...
- [Goals](goals.md)
- [User input and ask_user](user-input.md)
- [Sessions and SQLite storage](sessions.md)
- [Subagent implementation plan](subagents-implementation-plan.md)
