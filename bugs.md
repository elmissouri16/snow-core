...
- **Severity:** Medium
- **Surface:** Web Manager active-session snapshot presentation
- **Evidence:** After transcript consolidation, every accepted full runtime snapshot still entered multiple panel controllers. Several controllers synchronously published unchanged nested control objects or redrew unchanged attention, queue, steering, and inspection projections.
- **Fix:** Live-view controls compare the fields of bounded nested projections instead of fresh object identity. Attention compares its canonical request and effective busy/safety controls before drawing, takeover, or layout publication. Queue and steering continue to update admission authority synchronously but skip drawing semantically unchanged presentations. Inspection skips paint when its bounded live provider/model/session projection is unchanged. Snapshot revision, status, cancellation, permission, input, queue, goal, and mutation authority remain immediate; an animation-frame deferral was reviewed and deliberately rejected because it would temporarily leave stale mutation authority usable.
- **Verification:** Focused nested-control equality coverage, all 130 frontend tests, frontend typecheck, generated-asset reproducibility, the 92-assertion production live-stream workflow, all 51 transport client cases, the native React browser suite, affected and full Go tests, `go vet ./...`, all 70 Python support tests, the benchmark guard, targeted race coverage, `git diff --check`, and independent review pass. In the same bounded synthetic 100-message/128-activity/41-snapshot fixture used for BUG-249, attention controller time fell from 4.9 ms to 0.9 ms, queue from 1.6 ms to 0.4 ms, and steering from 2.5 ms to 0.7 ms; event-loop delay p95 fell from 3.7 ms to 2.2 ms and maximum delay from 64.5 ms to 61.3 ms. These synthetic figures show direction under a deliberately heavy fixture, not exact latency for a live user session.

## BUG-249: Live Web updates redundantly reconcile the transcript

- **Status:** Resolved — one accepted snapshot now produces at most one transcript commit.
- **Severity:** Medium
- **Surface:** Web Manager live conversation streaming and composer interaction
- **Evidence:** Every accepted full runtime snapshot separately called the message renderer and activity renderer even though both synchronously committed the same React transcript root. The snapshot then fanned out through the remaining runtime panels, including fields that did not change. A bounded production-fixture stress run with 100 messages, 128 activities, and 41 snapshots at the 75 ms coalescing interval measured two transcript commits per snapshot, a 65 ms browser long task, and 64.5 ms maximum event-loop delay. Live composer input also republished an unchanged workspace-draft projection synchronously.
- **Fix:** The transcript controller now projects messages and chronological activities together and commits them once, retains the existing compatibility APIs, and skips commits for semantically identical public projections while still observing changed runtime/edit/regenerate presentation capabilities. Unchanged workspace-draft projections no longer synchronously notify React consumers.
- **Verification:** Focused projection coverage verifies equality for fresh full-snapshot objects and detects changed message/activity content. The native message browser passes 132 React thumbnail assertions and 52 saved-history assertions, including combined-snapshot chronological activity placement and legacy capability changes. The production live-stream browser passes all 92 assertions. Frontend typecheck, all 130 frontend tests, reproducible production-asset checks, affected and full Go tests, `go vet ./...`, all 70 Python support tests, the benchmark guard, targeted race coverage, `git diff --check`, and independent review pass. A post-fix bounded rendered run observed 41 combined transcript updates for 41 accepted snapshots and zero legacy activity-only transcript calls.
...
- **Status:** Resolved — the exact-session recovery workflow completes repeatedly with fresh authority and no replay.
- **Severity:** Medium
- **Surface:** Web Manager explicit recovery after a worker dies after committing a write but before returning its tool result
- **Evidence:** The originally recorded browser run reached the `after-write` unknown-outcome boundary before activation returned 409 and the runtime read returned 404. Current investigation found a separate reproducible blocker at initial activation: the fixture worker's exact argument guard still required the retired `--no-mcp --no-subagents` profile while the manager now launches `--subagents` and permits configured MCP, and newly registered fixture projects inherited the enabled-skills default. The test worker therefore exited before opening any session. Once that fixture drift was corrected, the originally reported later-stage failure no longer reproduced on current source: the complete before-write and after-write sequence retains unresolved evidence, rotates instance authority and does not replay work.
- **Fix:** The real-browser fixture now explicitly disables project skills for its narrow permission scenario while validating the current managed worker arguments. Its kill marker must be successfully consumed before the worker exits, so a removal failure cannot leave a marker that immediately kills the replacement worker. A focused manager regression performs two consecutive failed-worker close/reopen cycles against the exact saved session and verifies fresh instances, retained admission evidence, empty live approval/activity authority and rejection of stale permission replies.
- **Verification:** Two consecutive complete production-browser permission workflows pass all 74 assertions, including isolated project-B completion, before-write and committed-after-write worker loss, exact-session reopen, unresolved durable tool history, one committed filesystem mutation and zero prompt/approval/write replay. The focused recovery tests pass 20 normal repetitions and 10 race-enabled repetitions; fixture guard, marker-consumption and real-write broker tests pass 10 repetitions. `go test ./internal/web ./cmd/snow -count=1`, `go test ./...`, and `go vet ./...` pass.
...
- **Status:** Resolved — React navigation retains sidebar search state and restores focus only when the user left it in the sidebar.
- **Regression evidence:** The permission-policy fixture's native workspace extraction attempted `.match` directly on a `Buffer`, so the desktop sidebar matrix stopped before exercising navigation. After converting the bounded exported document to text for extraction, the then-current production bundle consistently failed destination-row focus, preservation of open search/filter state, and the no-focus-pullback assertion after the user left the sidebar. The other seven sidebar checks passed, including scroll preservation, latest-selection cancellation, late-response rejection, failed-navigation live-state retention and content fallback focus.
- **Fix:** Initialize the remounted sidebar's search-focus transition guard from the retained controller state. An already-open search is no longer mistaken for a new user request to focus it, so the saved destination row wins when sidebar focus was captured and focus remains outside the sidebar when the user moved it during a delayed request. The browser regression now enters search text through native Chrome input rather than assigning a React-controlled value synthetically.
- **Historical evidence:** The user reported erratic workspace/session-list focus. Whole-workspace HTMX swaps replaced the sidebar, resetting list scroll and search, while the former classic owner unconditionally focused content. Sidebar links had independent request lifetimes, allowing older reads to finish after a newer selection. The former `syncNewSession` path rewrote unchanged current-row text/attributes on runtime updates. Sidebar Rename launched through an intermediate header menu, so its dialog returned focus to that header instead of the row. Before-swap cleanup also retired live state when HTMX explicitly declined a failed response swap.
- **Original remediation:** Synchronize sidebar reads with latest-selection-wins semantics; preserve list/search state and actual desktop sidebar focus across successful swaps; avoid unchanged row mutations; reuse the conversation owner's rename admission with the original row launcher; skip teardown for rejected swaps. Retain mobile content focus, native focus indication, explicit activation and runtime identity/admission checks. The first-party navigation and React shell owners now implement these boundaries, including the remount transition fix above.
- **Verification:** The current production React bundle passes the complete permission/sidebar matrix: **1,686 assertions** across all eight viewport/height/theme reports, plus the focused desktop run's **219 assertions**. Coverage verifies unchanged sidebar mutations, row-specific Rename focus return, native search text and scroll preservation, destination-row focus, cancellation of older reads, no late response/history overwrite, no focus pullback after leaving the sidebar, a still-live composer/subscription after a rejected navigation and content fallback focus. Frontend typecheck, all **128** unit tests, production build and generated-asset reproducibility pass; `go test ./...`, `go vet ./...`, all 70 Python tests and the benchmark guard also pass.
